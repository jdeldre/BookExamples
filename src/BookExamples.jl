...
function setup_grid()
  ...
  xmin = -maximum(r)/sqrt(2); xmax = maximum(r)/sqrt(2);
  ymin = -maximum(r)/sqrt(2); ymax = maximum(r)/sqrt(2);
  Z = r * exp.(im*θ');
...
function set_contour_levels(ψ,vmin::Float64,vmax::Float64,n::Int64)
...
function set_contour_levels(ψ)
  ...
  vmin = minimum(ψ);
  vmax = maximum(ψ);
  v = set_contour_levels(ψ,vmin,vmax,31)
...
function plot_streamlines(a::Vector{Complex128})
  ...
  ψmin = minimum(ψ[abs.(Z).>1.0])
  ψmax = maximum(ψ[abs.(Z).>1.0])


  pc = PyPlot.contour(real(Z),imag(Z),ψ,
                      set_contour_levels(ψ,ψmin,ψmax,21),
